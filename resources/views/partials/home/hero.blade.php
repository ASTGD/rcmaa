...
<section data-hero data-theme="dark" data-parallax-scope
    ...
    <div data-hero-media class="absolute inset-0">
        <div data-hero-mask class="absolute inset-0 will-change-[clip-path]">
            ...
            @if ($sources->isNotEmpty())
                <video data-hero-video class="absolute inset-0 h-full w-full object-cover brightness-125 saturate-110"
                    autoplay muted loop playsinline preload="metadata" poster="{{ $poster }}" aria-hidden="true"
                    tabindex="-1">
        ...
        <div class="absolute inset-0 hidden bg-gradient-to-r from-ink-950/85 via-ink-950/45 to-transparent md:block">
        </div>
    ...
    <div data-hero-content class="container-rc relative z-10 py-24 md:py-28">
        ...
        <h1 class="heading-display mt-7 max-w-[19ch] text-[clamp(2.4rem,5.6vw,4.6rem)] text-parchment" data-hero-title>
        ...
        @endphp
        @if ($hasNotPassed)
            <div class="mt-12 inline-flex flex-col gap-5 rounded-2xl border border-white/12 bg-ink-950/85 px-5 py-5 shadow-[0_24px_60px_-30px_rgba(0,0,0,.9)] backdrop-blur-md sm:px-7 lg:flex-row lg:items-left lg:gap-8"
                ...
                <div class="flex items-start gap-3 sm:gap-4">
                    @foreach ([['days', 'Days'], ['hours', 'Hours'], ['minutes', 'Minutes'], ['seconds', 'Seconds']] as [$unit, $label])
                        <div class="min-w-[3.1rem] text-center sm:min-w-[3.6rem]">
                            <span
                                class="heading-display block text-[2.1rem] font-semibold leading-none text-brass-500 tabular-nums [text-shadow:0_2px_18px_rgba(0,0,0,.55)] sm:text-[2.6rem]"
                        ...
                        @unless ($loop->last)
                            <span class="heading-display pt-0.5 text-2xl leading-none text-brass-500/35 sm:text-3xl"
    ...
    <a href="#about" class="absolute bottom-8 left-1/2 z-10 hidden -translate-x-1/2 flex-col items-center gap-2 md:flex"
        ...
        <span class="relative h-10 w-px overflow-hidden bg-white/15">
            <span
                class="absolute inset-x-0 top-0 h-1/2 animate-[scrollcue_2s_ease-in-out_infinite] bg-brass-500"></span>
        </span>
    </a>
