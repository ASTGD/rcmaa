<footer data-theme="dark" class="relative overflow-hidden bg-ink-950 text-ink-200">
    ...
    <div class="container-rc relative">
        ...
        <div class="grid gap-12 py-14 md:grid-cols-2 lg:grid-cols-[1.6fr_1fr_1fr_1.2fr]">
            ...
            @foreach ([
                'Explore' => [
                    ['Home', 'home'], ['Events', 'events.index'], ['Gallery', 'gallery'],
                    ['Notice', 'notices.index'], ['Directory', 'directory'],
                ],
                'Association' => [
                    ['About RCMAA', 'about'], ['Our Goal', 'our-goal'], ['Committee', 'committee'],
                    ['Faculty', 'teachers'], ['How to Apply', 'how-to-apply'],
                ],
            ] as $heading => $links)
        ...
        {{-- Legal strip --}}
        <div class="flex flex-col gap-4 border-t border-white/8 py-7 text-xs text-ink-400 sm:flex-row sm:items-center sm:justify-between">
            <p>&copy; {{ now()->year }} {{ config('rcmaa.name') }}. All rights reserved.</p>
            <ul class="flex flex-wrap items-center gap-x-6 gap-y-2">
                @foreach ([
                    ['Contact Us', 'contact'], ['Features', 'features'],
                    ['Privacy Policy', 'privacy'], ['Terms of Service', 'terms'], ['Help Center', 'help-center'],
                ] as [$label, $route])
