<x-admin-layout :title="'Registration '.$registration->reference">
    ...
    <div class="grid gap-6 lg:grid-cols-[1fr_22rem] lg:items-start">
        ...
        <div class="space-y-6">
            ...
            <div class="card p-6">
                <div class="flex flex-wrap items-start gap-5">
                    <span class="grid h-24 w-20 flex-none place-items-center overflow-hidden rounded-xl bg-ink-900 text-2xl font-semibold text-brass-500">
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <div class="mt-4 flex flex-wrap gap-x-6 gap-y-2 text-sm">
                            <a href="tel:{{ $registration->mobile }}" class="flex items-center gap-2 text-ink-600 transition hover:text-brass-700">
                                <x-icon name="phone" class="h-4 w-4 text-brass-600"/>{{ $registration->mobile }}
                            </a>
                            <a href="mailto:{{ $registration->email }}" class="flex items-center gap-2 text-ink-600 transition hover:text-brass-700">
            ...
            @foreach ([
                ...
                <div class="card overflow-hidden">
                    <h3 class="border-b border-ink-900/8 bg-parchment-dim px-6 py-3 font-mono text-[0.62rem] uppercase tracking-[0.16em] text-ink-500">
                        {{ $heading }}
                    </h3>
                    ...
                    <ul class="divide-y divide-ink-900/6">
                        @foreach ($registration->guests as $guest)
                            <li class="flex flex-wrap items-baseline gap-x-3 px-6 py-3 text-sm">
                                ...
                                @if (! empty($guest['occupation']))<span class="text-ink-400">&middot; {{ $guest['occupation'] }}</span>@endif
                            </li>
                        @endforeach
                    </ul>
                </div>
        ...
        <div class="space-y-4 lg:sticky lg:top-24">
            <div class="card overflow-hidden">
                <h3 class="border-b border-ink-900/8 bg-parchment-dim px-5 py-3 font-mono text-[0.62rem] uppercase tracking-[0.16em] text-ink-500">
                    Payment
                </h3>
                <dl class="divide-y divide-ink-900/6 text-sm">
                    @foreach ([
                        ...
                        'Amount due' => 'BDT '.number_format($registration->amount_due),
                    ] as $label => $value)
            ...
            <form method="POST" action="{{ route('admin.registrations.update', $registration) }}" class="card p-5">
                ...
                <div class="mt-4">
                    <label for="admin_note" class="field-label !text-xs">
                        Note to the registrant <span class="font-normal text-ink-400">(shown on their status page)</span>
                    </label>
